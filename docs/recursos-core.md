...
```bash
php artisan make:event NovoEvento
```
...
```bash
php artisan make:listener NovoListener
```
...
class NovoListener
{
    public function handle(NovoEvento $event)
    {
        ...
        info('Evento recebido: ' . json_encode($event->data));
    }
}
```
...
Você deve registrar o evento e seu listener no arquivo `EventServiceProvider.php`:
